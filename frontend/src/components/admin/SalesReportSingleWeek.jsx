...
function SalesReportSingleWeek() {
    ...
    const getOrder = async (number) => {
      ...
      instanceAdmin.get(`/weeklyorders/${number}`).then((response) => {
        if (response.data) {
          setspin(false);
          setnumber(number);
    ...
    const getOrders = async () => {
      try {
        ...
        if (number) {
          ...
          instanceAdmin.get(`/weeklyorders/${1}`).then((response) => {
            if (response.data) {
              setspin(false);
              setnumber(number);
              console.log(response.data);
...
export default SalesReportSingleWeek
